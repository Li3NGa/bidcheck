...
def map_responses(graph: RequirementGraph, response_text: str) -> list[ResponseMatch]:
    out=[]; normalized=response_text.lower()
    for req in graph.requirements:
        terms=[x for x in re.findall(r"[\u4e00-\u9fff]{2,}|[A-Za-z0-9]{3,}",req.text.lower()) if x not in {"投标","要求","提供","项目"}]
        hits=sum(1 for t in terms if t in normalized); score=min(1.0,hits/max(1,min(5,len(terms))))
        status="matched" if score>=0.6 else "review" if score>0 else "unmatched"
        excerpt=""
        for term in terms:
            pos=normalized.find(term)
            if pos>=0: excerpt=response_text[max(0,pos-80):pos+max(120,len(term))]; break
        out.append(ResponseMatch(req.id,status,excerpt,score))
    return out

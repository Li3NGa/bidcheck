...
class Match:
    requirement_id:str; status:str; evidence:str; confidence:float
...
def match_requirements(graph:RequirementGraph,response_text:str)->list[Match]:
    ...
    for req in graph.requirements:
        terms=[t for t in re.findall(r'[\u4e00-\u9fffA-Za-z0-9]{2,}',req.description) if len(t)>=2]
        hits=[t for t in terms if t in text]
        if not hits: results.append(Match(req.id,'missing','',0.0)); continue

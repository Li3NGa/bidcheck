...
def create_project(service:BidCheckService,payload:dict[str,Any],authorization:str|None=None,sessions:SessionStore|None=None)->dict[str,Any]:
    ...
    try: return service.create_project(TenderProject(str(payload['project_id']),str(payload['name']),graph,list(payload.get('response_documents',[])),payload.get('tenant_id')))
